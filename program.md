...
The training script runs for a **fixed time budget of 5 minutes** (wall clock training time, excluding startup/compilation). You launch it simply as: `uv run train.py`.
...
3. peak memory in GB, round to .1f (e.g. 12.3 -- divide peak_memory_mb by 1024) -- use 0.0 for crashes
...
```
commit	val_bpb	memory_gb	status	description
a1b2c3d	1.295000	13.7	keep	baseline (depth=4)
b2c3d4e	1.280000	13.9	keep	increase MATRIX_LR to 0.05
c3d4e5f	1.310000	13.7	discard	switch to GeLU activation
